<?php

/**
 * Template Name: Front Page
 *
 * Show pages info. You can set it under Appearance >> Customize.
 *
 * @package Mina olen
 * @subpackage Template
 * @since 1.0.0
 */

get_header(); // Loads the header.php template. ?>

<main <?php hybrid_attr( 'content' ); ?>>
	
	<?php $k = 1;
	
	while ( $k < apply_filters( 'mina_olen_how_many_pages', 7 ) ) : // Begins the loop through found posts from customize settings. 
	
		$mina_olen_page_content = absint( get_theme_mod( 'front_page_' . $k ) );
	
		if ( 0 != $mina_olen_page_content || !empty( $mina_olen_page_content ) ) : // Check if page is selected. ?>
	
			<article <?php hybrid_attr( 'post' ); ?>>
			
				<div class="wrapper-inner">
				
					<header class="entry-header">
						
						<?php if ( current_theme_supports( 'get-the-image' ) ) get_the_image( array( 'post_id' => $mina_olen_page_content, 'size' => 'mina-olen-thumbnail' ) ); ?>
						
						<h2 class="entry-title"><a href="<?php echo get_permalink( $mina_olen_page_content ); ?>"><?php echo get_the_title( $mina_olen_page_content ); ?></a></h2>
					
					</header><!-- .entry-header -->

					<div class="entry-summary">
						
						<?php if( has_excerpt( $mina_olen_page_content ) ) { 
							echo wpautop( get_post_field( 'post_excerpt', $mina_olen_page_content ) ); 
						} else {
							echo wpautop( wp_trim_words( get_post_field( 'post_content', $mina_olen_page_content ) ) );
						} ?>
						
						<p><span class="mina-olen-read-more"><a class="more-link" href="<?php echo get_permalink( $mina_olen_page_content ); ?>"><?php _e( 'Read more', 'mina-olen' ); ?></a></span></p>
						
					</div><!-- .entry-summary -->
				
				</div><!-- .wrapper-inner -->
					
			</article><!-- .entry -->
			
		<?php /* Is there even or odd number of pages selected. Need this for styling. */
	
		if( 0 == ( $k % 2 ) ) {
			$mina_olen_class_even_or_odd = 'even';
		} else {
			$mina_olen_class_even_or_odd = 'odd';
		}
		
		?>
		
		<?php endif; //End if page is selected. ?>
				
		<?php $k++; // Add one before loop ends. 
	
	endwhile; // End found posts loop.
	
	/* Show latest posts if checkbox is selected. */
	
	if( get_theme_mod( 'show_latest_posts' ) ) :
	
		/* Set custom query to show latest posts. */
		$mina_olen_post_args = apply_filters( 'mina_olen_front_page_latest_posts_arguments', array(
			'post_type'           => 'post',
			'posts_per_page'      => 3,
			'ignore_sticky_posts' => 1
		) );
			
		$mina_olen_posts = new WP_Query( $mina_olen_post_args );
	
		$mina_olen_latest_posts = esc_attr( apply_filters( 'mina_olen_front_page_latest_posts', __( 'Latest Posts', 'mina-olen' ) ) ); ?>
			
		<section id="mina-olen-latest-posts" class="entry <?php echo $mina_olen_class_even_or_odd; ?>">
		
		<h1 id="mina-olen-latest-posts-title"><?php printf( __( '%1$s', 'mina-olen' ), $mina_olen_latest_posts ); ?></h1>
	
		<?php if ( $mina_olen_posts->have_posts() ) : ?>

			<?php while ( $mina_olen_posts->have_posts() ) : $mina_olen_posts->the_post(); ?>

				<article <?php hybrid_attr( 'post' ); ?>>
				
					<div class="wrapper-inner">
				
						<header class="entry-header">
							<?php if ( current_theme_supports( 'get-the-image' ) ) get_the_image(); ?>
						</header><!-- .entry-header -->

						<div class="entry-summary">
							<?php the_title( '<h2 class="entry-title"><a href="' . get_permalink() . '">', '</a></h2>' ); ?>
							<?php the_excerpt(); ?>
							<?php wp_link_pages( array( 'before' => '<p class="page-links">' . '<span class="before">' . __( 'Pages:', 'mina-olen' ) . '</span>', 'after' => '</p>' ) ); ?>
						</div><!-- .entry-summary -->

					</div><!-- .wrapper-inner -->
				
				</article><!-- .entry -->
				
			<?php endwhile; ?>

		<?php endif; wp_reset_postdata(); // reset query ?>
		
		</section><!-- .entry -->
		
	<?php endif; // End check if user wants to show latest posts.
	
	/* Show latest portfolios if checkbox is selected. */
	
	if( get_theme_mod( 'show_latest_portfolios' ) ) :
	
		/* Set custom query to show latest portfolios. */
		$mina_olen_portfolio_args = apply_filters( 'mina_olen_front_page_latest_portfolios_arguments', array(
			'post_type'           => 'portfolio_item',
			'posts_per_page'      => 3
		) );
			
		$mina_olen_portfolios = new WP_Query( $mina_olen_portfolio_args );
	
		$mina_olen_latest_portfolios = esc_attr( apply_filters( 'mina_olen_front_page_latest_portfolios', __( 'Latest Portfolios', 'mina-olen' ) ) ); ?>
		
		<section id="mina-olen-latest-portfolios" class="entry <?php echo $mina_olen_class_even_or_odd; ?>">
		
		<h1 id="mina-olen-latest-portfolios-title"><?php printf( __( '%1$s', 'mina-olen' ), $mina_olen_latest_portfolios ); ?></h1>
	
		<?php if ( $mina_olen_portfolios->have_posts() ) : ?>

			<?php while ( $mina_olen_portfolios->have_posts() ) : $mina_olen_portfolios->the_post(); ?>

				<article <?php hybrid_attr( 'post' ); ?>>
				
					<div class="wrapper-inner">
				
						<header class="entry-header">
							<?php if ( current_theme_supports( 'get-the-image' ) ) get_the_image(); ?>
						</header><!-- .entry-header -->

						<div class="entry-summary">
							<?php the_title( '<h2 class="entry-title"><a href="' . get_permalink() . '">', '</a></h2>' ); ?>
							<?php the_excerpt(); ?>
							<?php wp_link_pages( array( 'before' => '<p class="page-links">' . '<span class="before">' . __( 'Pages:', 'mina-olen' ) . '</span>', 'after' => '</p>' ) ); ?>
						</div><!-- .entry-summary -->

					</div><!-- .wrapper-inner -->
				
				</article><!-- .entry -->
				
			<?php endwhile; ?>

		<?php endif; wp_reset_postdata(); // reset query ?>
		
		</section><!-- .entry -->
		
	<?php endif; // End check if user wants to show latest portfolios. ?>

</main><!-- #content -->

<?php get_footer(); // Loads the footer.php template. ?>